...
def interpolatedBaumWelchP(bitext_sd, s_count, t_table, sd_count, supervisedA, supervisedTProb, lWeight):#L is the number of observations
    ...
    for iterations in range(0,10):
        ...
        for I in target_length_map:
            for i in range(1,I+1):
                for j in range(1,I+1):
                    unsupervisedVal = totalC_j_Minus_iOverAllObservations[(i,j,I)]/totalC_l_Minus_iOverAllObservations[i,I]
                    a[(i, j, I)] = (1-p0H)*((lWeight * supervisedA[i, j, I]) + ((1 - lWeight) * unsupervisedVal))
        ...
        for k in range(sd_size):
            (f,e) = biword[k]
            unsupervisedVal = totalGammaDeltaOverAllObservations_t_overall_states[k]/totalGammaDeltaOverAllObservations_t_overall_states_over_dest[e]
            t_table[(f, e)] = (lWeight * supervisedTProb[(f, e)]) + ((1 - lWeight) * unsupervisedVal)
